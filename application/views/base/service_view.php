...
function get_oas_bget($orParams, $andParams) {
    ...
    $dbconn3 = pg_connect("host='localhost' port='5432' dbname='gpcan' user='postgres' password='%gpca$'");

    $andQuery = pg_fetch_all(pg_query(#$this->db->query(
        "SELECT lo_id, lo_xml_lom, ts_rank_cd(campo_busqueda_index_col, query) AS rank
